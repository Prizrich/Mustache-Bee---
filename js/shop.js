...
function buySpecificProduct(productId, quantity = 1) {
    ...
    if (gameState.money >= totalCost) {
        ...
        addExp(5 * quantity);
        playSound("buy");
        addMailMessage("system", `✅ Закуплен ${product.name} x${quantity} за ${totalCost}₽`, true);
        saveGameProgress();
        return { success: true, message: `Куплено ${quantity} шт. ${product.name}` };
    }
    return { success: false, message: `Не хватает денег! Нужно ${totalCost}₽` };
}

function setProductPriceWithAI(productId, newPrice) {
    const product = gameState.products.find(p => p.id === productId);
    if (!product || newPrice < 5) return { success: false, feedback: "Цена слишком низкая!" };
    product.userPrice = Math.floor(newPrice);
    let feedback = "", reputationChange = 0;
    if (newPrice > product.basePrice * 1.5) { feedback = "😡 Цена слишком высокая!"; reputationChange = -2; }
    else if (newPrice > product.basePrice * 1.2) { feedback = "😐 Цена завышена"; reputationChange = -1; }
    else if (newPrice < product.basePrice * 0.7) { feedback = "🎉 Отличная скидка!"; reputationChange = 3; addExp(5); }
    else if (newPrice < product.basePrice * 0.9) { feedback = "👍 Хорошая цена!"; reputationChange = 1; }
    else { feedback = "👌 Цена оптимальная"; reputationChange = 0; }
    if (reputationChange !== 0) updateReputation(reputationChange);
    addMailMessage("system", `💰 Цена на ${product.name}: ${newPrice}₽. ${feedback}`, true);
    saveGameProgress();
...
async function generateRandomReview() {
    ...
    const currentPrice = product?.userPrice || 100;
    ...
    const reviewText = generateReview(author, productName, isPositive, currentPrice);
    ...
    if (author === "Карась" && !isPositive) { addCompromat(2); addMailMessage("system", "📁 Компромат +2!", true); }
    return { author, rating, text: reviewText };
}

function replyToReviews() {
    const unreplied = gameState.reviews.filter(r => !r.responded);
    if (unreplied.length === 0) { addMailMessage("system", "Нет новых отзывов.", true); return false; }
    ...
    addMailMessage("system", `💬 Ответили на ${unreplied.length} отзывов! +${GAME_CONFIG.replyReputationBonus} репутации!`, true);
    playSound("win");
    saveGameProgress();
    return true;
}